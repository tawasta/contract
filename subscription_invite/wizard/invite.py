...
class SubscriptionInviteWizard(models.TransientModel):
    ...
    def action_send_invite(self):
        ...
        invite = self.env['subscription.invitation'].sudo().create({
        ...
        template = self.env.ref('subscription_invite.subscription_invitation_email_template')
        if not template:
            raise UserError("Email template not found")

        template.sudo().send_mail(line.id, force_send=True, email_values={'email_to': email})

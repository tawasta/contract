...
class SaleSubscriptionLine(models.Model):
    ...
    def _get_or_create_product_for_part(self, name):
        ...
        tmpl = Template.create({"name": name, "type": "service", "subscribable": True})
        _logger.info(
    ...
    def _sql_backfill_line_company(self, subscription_id, company_id, line_ids=None):
        ...
        _logger.info(
            "cron_split: SQL backfill company_id=%s (sub_id=%s, line_ids=%s) -> updated=%s",
            company_id,
            subscription_id,
    ...
    def cron_split_lines_by_semicolon(self, limit=500, line_id=None):
        ...
        for line in lines:
            ...
            if not subscription:
                skipped += 1
                _logger.info("cron_split: SKIP line_id=%s (no subscription)", line.id)
                continue
            ...
            if not target_company:
                skipped += 1
                _logger.info("cron_split: SKIP line_id=%s (no target company)", line.id)
                continue

            if not subscription.company_id:
                subscription.write({"company_id": target_company.id})
                _logger.info("cron_split: set subscription.company_id -> %s (sub_id=%s)", target_company.id, subscription.id)

            # FIX: backfill NULL stored related company_id on ALL lines of this subscription
            self._sql_backfill_line_company(subscription.id, subscription.company_id.id, line_ids=None)

            # Re-browse to ensure we see fresh DB values after SQL
            line = Line.browse(line.id)
            subscription = line.sale_subscription_id
            ...
            if len(parts) < 2:
                skipped += 1
                _logger.info("cron_split: SKIP line_id=%s parts=%s (<2)", line.id, parts)
                continue
            ...
            _logger.info(
                "cron_split: update ORIGINAL line_id=%s line_company_id=%s sub_company_id=%s -> desc='%s' product='%s'",
                line.id,
                line.company_id.id if line.company_id else None,
                subscription.company_id.id if subscription.company_id else None,
                first_part,
            ...
            for idx, part in enumerate(parts[1:], start=2):
                ...
                # Backfill company_id for new line too (if it somehow landed NULL)
                ...
                _logger.info(
                    "cron_split: created NEW line %s/%s new_line_id=%s new_line_company_id=%s sub_id=%s desc='%s' product='%s'",
                    idx,
                    len(parts),
                    new_line.id,
